...
- (void)cancelOnRequestThread
{
     ...
     *
     * if you manage the values for the isFinished and isExecuting properties yourself (perhaps because you are implementing a concurrent operation), you must
     * update those variables accordingly. Specifically, you must change the value returned by isFinished to YES and the value returned by isExecuting to NO.
     * You must make these changes even if the operation was cancelled before it started executing.
     */
    // Last line in documentation is wrong, see http://stackoverflow.com/a/9500329
    // If operation not started, set finish=YES in [operation start] if cancelled
    // If operation has started, set finish=YES, executing = YES in [operation cancel]
    // ASIHTTPRequest will set error in failWithError:, if not executing, then will markAsFinished inside main,
    // otherwise markAsFinished which set finished=YES and executing = NO
    // REFER TO LINE 2113
...
- (void)failWithError:(NSError *)theError
{
    ...
    // Edited: If cached response redirects more than it should, then do not load it from cache again
    // Edited: Cancelled request should not load from cache
	if ([self downloadCache] && ([self cachePolicy] & ASIFallbackToCacheIfLoadFailsCachePolicy)
        && self.redirectCount <= RedirectionLimit * 2
        && theError != ASIRequestCancelledError) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
